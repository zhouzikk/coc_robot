    ...
    def 执行单次搜索(self, 上下文: 任务上下文, 搜索次数: int, 最小资源: int) -> bool:
        ...
        if 资源数据["总资源"] >= 最小资源:
            ...
            if self.是否有足够资源建筑靠近边缘(上下文, 80, 最少靠近比例=靠近比例):
                print("是否出现17本图", self.已出现图片("避免_17本1.bmp|避免_17本2.bmp|避免_17本3.bmp|避免_17本4bmp|避免_17本5.bmp|避免_17本.bmp"))
                if self.已出现图片("避免_17本1.bmp|避免_17本2.bmp|避免_17本3.bmp|避免_17本4bmp|避免_17本5.bmp|避免_17本.bmp"):
                    上下文.置脚本状态("成功找到敌人")
                    return True
                else:
                    上下文.置脚本状态("对方17本，不进攻，搜索下一个")
