...
class 进攻任务(基础任务):
    ...
    def 执行(self, 上下文: 任务上下文) -> bool:
        ...
        兵种模板列表 = [
            ...
            "兵种_雷龙.bmp",
            "兵种_飞龙.bmp",
            "英雄_弓箭女皇.bmp", "英雄_亡灵王子.bmp", "英雄_野蛮人之王.bmp", "英雄_飞盾战神.bmp",
            "兵种_大雪怪.bmp",
            "兵种_天使.bmp",
            "兵种_天狗.bmp",
            "兵种_女巫.bmp",
        ...
        #兵种模板列表 = ["兵种_气球兵.bmp","兵种_飞龙.bmp", "兵种_雷龙.bmp","英雄_弓箭女皇.bmp","英雄_亡灵王子.bmp","英雄_野蛮人之王.bmp","英雄_飞盾战神.bmp"]
        有效目标数量 = len(有效目标列表)
        ...
        for 兵种路径 in 兵种模板列表:
            ...
            while True:  # 加个最大重试次数防止死循环
                ...
                上一次下兵已成功 = self.下兵(上下文, 兵种路径, 目标坐标)
                if not 上一次下兵已成功:
                    失败次数 += 1
                else:
                ...
                #上下文.置脚本状态(f"{兵种路径} 未能成功对任一目标下兵")

        # # 批量下飞龙兵种
        # for 目标 in 有效目标列表:
        #     上下文.置脚本状态(f"进攻目标点{目标['中心坐标'].x},{目标['中心坐标'].y}, 类别为{目标['类别名称']}")
        #     self.下兵(上下文, "兵种_飞龙.bmp", 目标['中心坐标'])
        #
        # # 批量下飞龙兵种
        # for 目标 in 有效目标列表:
        #     上下文.置脚本状态(f"进攻目标点{目标['中心坐标'].x},{目标['中心坐标'].y}, 类别为{目标['类别名称']}")
        ...
        # def 执行下兵流程(兵种名称: str):
        #     失败次数 = 0
        #     while 有效目标列表 and 失败次数 < 3:
        #         随机目标 = random.choice(有效目标列表)
        #         if self.下兵(上下文, 兵种名称, 随机目标['中心坐标']):
        #             失败次数 = 0
        #         else:
        #             失败次数 += 1
        #             #上下文.置脚本状态(f"[警告] {兵种名称}下兵失败次数：{失败次数}")
        #     if 失败次数 >= 3:
        #         上下文.置脚本状态(f"[终止] {兵种名称}连续3次失败,可能已下完,或者没有这个英雄出战")
        #
        # # 执行英雄下兵流程
        # 执行下兵流程("英雄_弓箭女皇.bmp")
        # 执行下兵流程("英雄_亡灵王子.bmp")
    ...
    def 下兵(self, 上下文: 任务上下文, 兵种路径: str, 中心坐标: 坐标) -> bool:
