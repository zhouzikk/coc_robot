...
class 城墙升级任务(基础任务):
    ...
    def 检查升级条件(self, 上下文, x1, y1, x2, y2) -> bool:
        ...
        try:
            ...
            #
            #
            # 掩码 = cv2.inRange(区域图像, (0xFA, 0x87, 0x7C), (0xFA, 0x87, 0x7C))
            # 白色像素数 = cv2.countNonZero(掩码)
    ...
    def 执行升级(self, 上下文, x1, y1, x2, y2) -> bool:
        ...
        try:
            ...
            是否有金币图标,(金币x,金币y),金币调试图=self.模板识别.执行匹配(区域图像,"升级建筑的金币小图标.bmp",调试模式=True)
            是否有金币图标, (圣水x, 圣水y), _ = self.模板识别.执行匹配(区域图像, "升级建筑的圣水小图标.bmp")
            金币x=276+金币x-27
            金币y=440+金币y+30
            ...
            # cv2.rectangle(屏幕图像, (金币x, 金币y), (金币x+10, 金币y+10), (0, 255, 0), 2)
            # cv2.imshow("a",屏幕图像)
            # cv2.waitKey(0)
            # cv2.destroyAllWindows()
