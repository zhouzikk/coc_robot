...
class 城墙升级任务(基础任务):
    ...
    def 已够资源升级(self)-> bool:
        ...
        if 当前金币>刷墙起始金币 or 当前圣水>刷墙起始圣水:
            上下文.置脚本状态("资源满足用户设定的条件,开始刷墙",20*2)
            return True
        else:
    ...
    def 执行升级(self, 上下文, x1, y1, x2, y2) -> bool:
        ...
        try:
            ...
            有金币图标,(金币x,金币y),金币调试图=self.模板识别.执行匹配(区域图像,"升级建筑的金币小图标.bmp",)
            有圣水图标, (圣水x, 圣水y), _ = self.模板识别.执行匹配(区域图像, "升级建筑的圣水小图标.bmp",)
            金币x=276+金币x-27
            金币y=440+金币y+30
