...
class 任务数据库:
    ...
    # ==== 运行数据 ====
    def 置当前任务状态(self, 用户标识: str, 当前任务: str, 状态: str):
        """更新运行时状态"""
        with self._获取连接() as conn:
            conn.execute("""
...
class 任务上下文:
    ...
    当前坐标: tuple = (0, 0)
    战斗状态: str = "待机"
    #背包物品: dict = field(default_factory=dict)
...
class 用户任务线程:
    ...
    def 开始工作(self):
        ...
        self.主线程.start()
        self.状态 = "运行中"
        self.数据库.置当前任务状态(self.用户标识, "初始化", "运行中")
    ...
    def _任务流程(self):
        ...
        try:
            ...
            #当用户线程的_任务流程方法抛出未捕获异常时：
            #异常会导致线程主循环终止，不再执行_记录心跳()
            #然后会被监控中心检测到无心跳，导致重启
            self.状态 = "异常"
            self.数据库.置当前任务状态(self.用户标识, "异常处理", "异常状态")
    ...
    def _执行阶段(self):
        ...
        # 模拟任务处理
        self.数据库.置当前任务状态(self.用户标识, "数据处理", "运行中")
        time.sleep(当前设置.任务参数.get("处理间隔", 1.0))

        self.数据库.置当前任务状态(self.用户标识, "计算任务", "运行中")
    ...
    def _记录心跳(self):
        ...
        # 修正方法名
        self.数据库.置当前任务状态(self.用户标识, "心跳记录", "正常")
    ...
    def 停止(self):
        ...
        if self.主线程.is_alive():
            self.主线程.join()
        self.状态 = "已停止"
        self.数据库.置当前任务状态(self.用户标识, "停止", "已停止")
